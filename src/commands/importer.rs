...
use crate::utils::{id_for_account, id_for_category, apply_import_rules};
...
fn import_transactions(conn: &Connection, sub: &clap::ArgMatches) -> Result<()> {
    ...
    for result in rdr.records() {
        ...
        let mut cat_id = if category.is_empty() { None } else { Some(id_for_category(conn, &category)?) };
        if cat_id.is_none() {
            let (rule_cat, rewrite) = apply_import_rules(conn, &payee, if note.is_empty(){None}else{Some(&note)})?;
            if cat_id.is_none() { cat_id = rule_cat; }
            if let Some(newp) = rewrite { payee = newp; }
        }
        let ccy: String = if currency.is_empty() {
            conn.query_row("SELECT currency FROM accounts WHERE id=?1", params![acct_id], |r| r.get::<_, String>(0))?
        } else { currency };

        conn.execute(
            "INSERT INTO transactions(date, account_id, amount, payee, category_id, currency, note)
             VALUES (?1,?2,?3,?4,?5,?6,?7)",
            params![date, acct_id, amount, payee, cat_id, ccy, if note.is_empty() { None::<String> } else { Some(note) }],
        )?;
    }

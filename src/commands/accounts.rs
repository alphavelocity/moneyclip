...
pub fn handle(conn: &Connection, m: &clap::ArgMatches) -> Result<()> {
    match m.subcommand() {
        Some(("add", sub)) => {
            ...
            conn.execute("INSERT INTO accounts(name, type, currency) VALUES (?1, ?2, ?3)", params![name, typ, ccy])?;
            println!("Added account '{}' ({}, {})", name, typ, ccy);
        }
        Some(("list", _)) => {
            let mut stmt = conn.prepare("SELECT name, type, currency, created_at FROM accounts ORDER BY name")?;
            ...
            println!("{}", pretty_table(&["Name","Type","Currency","Created"], data));
        }
        Some(("rm", sub)) => {
            let name = sub.get_one::<String>("name").unwrap();

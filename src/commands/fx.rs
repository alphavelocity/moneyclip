...
use crate::utils::{get_base_currency, set_base_currency, http_client, pretty_table};
...
fn fetch_rates(conn: &Connection, days: usize) -> Result<()> {
    ...
    if targets.is_empty() { println!("No non-base currencies found; nothing to fetch."); return Ok(()); }
...
fn list_rates(conn: &Connection) -> Result<()> {
    let mut stmt = conn.prepare("SELECT date, base, quote, rate FROM fx_rates ORDER BY date DESC, base, quote LIMIT 50")?;
    ...
    for row in rows { let (d,b,q,r) = row?; data.push(vec![d,b,q,r]); }

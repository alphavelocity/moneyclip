...
use crate::utils::{http_client, id_for_account, id_for_asset, parse_date, parse_decimal, pretty_table};
...
fn add_asset(conn: &Connection, sub: &clap::ArgMatches) -> Result<()> {
    ...
    conn.execute("INSERT INTO assets(ticker, name, currency) VALUES (?1,?2,?3)", params![ticker, name, currency])?;
...
fn list_assets(conn: &Connection) -> Result<()> {
    ...
    for row in rows { let (t,n,c)=row?; data.push(vec![t,n,c]); }
...
fn record_trade(conn: &Connection, sub: &clap::ArgMatches, side: &str) -> Result<()> {
    ...
    conn.execute(
        ...
        params![date.to_string(), asset_id, account_id, qty.to_string(), price.to_string(), fees.to_string(), side],
    )?;
    println!("Recorded {} {} x {} @ {} (fees {})", side, qty, ticker, price, fees);
    Ok(())
}

fn value(conn: &Connection, sub: &clap::ArgMatches) -> Result<()> {
    let live = sub.get_flag("live");
    if live { fetch_prices(conn)?; }
    ...
    let rows = stmt.query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?, r.get::<_, String>(2)?, r.get::<_, f64>(3)?)))?;
    let mut data = Vec::new();
    for row in rows {
        let (ticker, ccy, px_s, qty_f) = row?;
        let px: f64 = px_s.parse().unwrap_or(0.0);
        let val = px * qty_f;
        data.push(vec![ticker, ccy, format!("{:.4}", qty_f), format!("{:.2}", px), format!("{:.2}", val)]);
    }
    println!("{}", pretty_table(&["Ticker","CCY","Qty","Price","Value"], data));
    Ok(())
}
...
fn tax_cg(conn: &Connection, sub: &clap::ArgMatches) -> Result<()> {
    ...
    let sells = stmt.query_map([year], |r| Ok((
        r.get::<_, String>(0)?,
        r.get::<_, String>(1)?,
        r.get::<_, String>(2)?,
        r.get::<_, String>(3)?,
    ...
    for sell in sells {
        ...
        let mut remaining = s_qty_s.parse::<rust_decimal::Decimal>().unwrap_or(rust_decimal::Decimal::ZERO);
        let sell_price = s_price_s.parse::<rust_decimal::Decimal>().unwrap_or(rust_decimal::Decimal::ZERO);
        let sell_fees = s_fees_s.parse::<rust_decimal::Decimal>().unwrap_or(rust_decimal::Decimal::ZERO);
        let mut lots = lot_stmt.query_map([&ticker], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?, r.get::<_, String>(2)?, r.get::<_, String>(3)?)))?;
        ...
        while remaining > rust_decimal::Decimal::ZERO {
            ...
            if let Some(l) = next_row {
                ...
                let mut b_qty = b_qty_s.parse::<rust_decimal::Decimal>().unwrap_or(rust_decimal::Decimal::ZERO);
                let b_price = b_price_s.parse::<rust_decimal::Decimal>().unwrap_or(rust_decimal::Decimal::ZERO);
                let b_fees = b_fees_s.parse::<rust_decimal::Decimal>().unwrap_or(rust_decimal::Decimal::ZERO);
                if b_qty.is_zero() { continue; }
    ...
    println!("{}", pretty_table(&["Ticker","Sell Date","CCY","Realized Gain"], table_rows));
    Ok(())
}

#[derive(Debug, Deserialize)]
#[allow(non_snake_case)]
struct YahooResponse { quoteResponse: QuoteResponse }
#[derive(Debug, Deserialize)]
#[allow(non_snake_case)]
struct QuoteResponse { result: Vec<YahooQuote> }
#[derive(Debug, Deserialize)]
struct YahooQuote {
    #[serde(rename = "regularMarketPrice")] regular_market_price: Option<f64>,
...
fn list_prices(conn: &Connection) -> Result<()> {
    ...
    let mut data = Vec::new();
    for row in rows { let (tic, ts, px, ccy, src) = row?; data.push(vec![tic, ts, px, ccy, src]); }
    println!("{}", pretty_table(&["Ticker","As Of","Price","CCY","Source"], data));
    Ok(())
}
...
fn fetch_prices(conn: &Connection) -> Result<()> {
    ...
    let mut map = Vec::new();
    for row in rows { let (id, tic) = row?; tickers.push(tic.clone()); map.push((tic, id)); }
    if tickers.is_empty() { println!("No assets to fetch"); return Ok(()); }

    let url = format!("https://query1.finance.yahoo.com/v7/finance/quote?symbols={}", tickers.join(","));

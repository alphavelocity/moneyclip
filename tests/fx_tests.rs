...
fn fx_triangulation_and_reciprocal() {
    ...
    conn.execute("INSERT INTO fx_rates(date,base,quote,rate) VALUES (?1,?2,?3,?4)", params!["2025-08-01","USD","INR","83"]).unwrap();
    conn.execute("INSERT INTO fx_rates(date,base,quote,rate) VALUES (?1,?2,?3,?4)", params!["2025-08-01","USD","EUR","0.90"]).unwrap();

    // EUR 90 -> INR ? via USD hub
    let amt = Decimal::new(9000, 2);
    let res = moneyclip::utils::fx_convert(&conn, NaiveDate::from_ymd_opt(2025,8,15).unwrap(), amt, "EUR", "INR").unwrap();
    ...
    let res2 = moneyclip::utils::fx_convert(&conn, NaiveDate::from_ymd_opt(2025,8,15).unwrap(), amt_inr, "INR", "USD").unwrap();
    // 166 / 83 = 2.0 USD

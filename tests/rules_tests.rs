...
fn setup() -> Connection {
    ...
    "#).unwrap();
    conn.execute("INSERT INTO categories(name) VALUES('Shopping')", []).unwrap();
    conn
}
...
fn rule_applies_regex_and_rewrite() {
    let conn = setup();
    let cat_id: i64 = conn.query_row("SELECT id FROM categories WHERE name='Shopping'", [], |r| r.get(0)).unwrap();
    ...
    let (c, r) = moneyclip::utils::apply_import_rules(&conn, "AMZN Mktp US*AB123", Some("order 123")).unwrap();
